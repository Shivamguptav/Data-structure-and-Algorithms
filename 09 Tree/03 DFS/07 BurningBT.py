...
class BurningTree:
    ...
    def burningTree(self, root, targetNode):
        self.queue = deque([])  # Initialize the queue inside the method
        if root is None:
            return 0
        if root.data == targetNode:
            print(root.data)
        ...
        left = self.burningTree(root.left, targetNode)
        if left == 1:
            self.printAndAddChildren(root.right)
            print(root.data)
            return 1

        right = self.burningTree(root.right, targetNode)
        if right == 1:
            self.printAndAddChildren(root.left)
            print(root.data)
            return 1

        return 0

    def printAndAddChildren(self, node):
        if node:
            queueSize = len(self.queue)
            for i in range(queueSize):
                current = self.queue.popleft()
